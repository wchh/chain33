...
func Test_sendQueryData(t *testing.T) {
	...
	_, ok := proto.handleSend(&types.P2PQueryData{}, testPid)
	assert.True(t, ok)
}
...
func Test_sendQueryReply(t *testing.T) {
	...
	proto := newTestProtocol()
	_, ok := proto.handleSend(&types.P2PBlockTxReply{}, testPid)
	assert.True(t, ok)
}
...
func Test_recvQueryData(t *testing.T) {
	...
	sendData, _ := proto.handleSend(query, testPid)
...
func Test_recvQueryReply(t *testing.T) {
	...
	sendData, _ := proto.handleSend(reply, testPid)
	err := proto.handleReceive(sendData, nil, testPid)
